  ...
  pub fn create(&self) -> Result<String, String> {
    ...
    let s = format!(
      ...
      pub fn new(gui: &ImGui) -> Font {{
        ...
        let mut stage = vk::mem::Staging::new(&mut gui.alloc.clone(), ({dimx} * {dimy}) as vk::DeviceSize).unwrap();
        ...
        let mut batch = vk::cmd::AutoBatch::new(gui.device).unwrap();
