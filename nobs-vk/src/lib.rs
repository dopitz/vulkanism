...
///
/// # Details
/// This trait only requires the implementation of [get_extensions](trait.InstanceWrapper.html#tymethod.get_extensions).
/// Instance level vulkan commands are then passed through to their respective core or instance extension function pointer.
/// If the vulkan command required an `Instance` parameter, the wrapper will drop this parameter, since the
/// instance handle can be provided by the [InstanceExtensions](struct.InstanceExtensions.html).
///
/// # Examples
///
/// ```
...
///
/// # Details
/// This trait only requires the implementation of [get_extensions](trait.DeviceWrapper.html#tymethod.get_extensions).
/// Device level vulkan commands are then passed through to their respective core or device extension function pointer.
/// If the vulkan command required a `Device` parameter, the wrapper will drop this parameter, since the
/// device handle can be provided by the [DeviceExtensions](struct.DeviceExtensions.html).
///
/// Example
/// The DeviceWrapper uses the same pattern as the [InstanceWrapper](trait.InstanceWrapper.html), look [here for an example](trait.InstanceWrapper.html#examples).
